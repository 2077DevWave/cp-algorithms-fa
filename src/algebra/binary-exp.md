...
این روش همچنین کاربردهای مهمی در بسیاری از مسائل غیرمرتبط با محاسبات توان دارد، زیرا می‌توان آن را با هر عملیاتی که خاصیت **شرکت‌پذیری** دارد، استفاده کرد:
...
$$a^n = \begin{cases}
1 &\text{if } n == 0 \\
\left(a^{\frac{n}{2}}\right)^2 &\text{if } n > 0 \text{ , } n \text{ is even}\\
\left(a^{\frac{n - 1}{2}}\right)^2 \cdot a &\text{if } n > 0 \text{ , } n \text{ is odd}\\
\end{cases}$$
...
اگرچه پیچیدگی هر دو رویکرد یکسان است، ولی این رویکرد در عمل سریع‌تر خواهد بود زیرا سربار فراخوانی‌های بازگشتی را نداریم.
...
**مسئله:**
محاسبه $x^n \bmod m$.
این یک عملیات بسیار رایج است. به عنوان مثال در محاسبه [وارون ضرب پیمانه‌ای](module-inverse.md) استفاده می‌شود.
...
این موضوع مستقیماً از قضیه کوچک فرما و قضیه اویلر نتیجه می‌شود، برای جزئیات بیشتر به مقاله [وارون ضرب پیمانه‌ای](module-inverse.md#fermat-euler) مراجعه کنید.
...
**مسئله:** با داشتن $n$ نقطه $p_i$، به تعداد $m$ بار تبدیل را بر روی هر یک از این نقاط اعمال کنید. هر تبدیل می‌تواند یک انتقال، یک مقیاس‌دهی یا یک دوران حول یک محور معین با یک زاویه معین باشد. همچنین یک عملیات "حلقه (loop)" وجود دارد که یک لیست معین از تبدیل‌ها را $k$ بار اعمال می‌کند (عملیات "حلقه" می‌توانند تودرتو باشند). شما باید تمام تبدیل‌ها را سریع‌تر از $O(n \cdot length)$ اعمال کنید، که در آن $length$ تعداد کل تبدیل‌هایی است که باید اعمال شوند (پس از باز کردن "حلقه").
...
(شاید بپرسید چرا یک مختصات چهارم ساختگی معرفی کردیم؟ این زیبایی [مختصات همگن](https://en.wikipedia.org/wiki/Homogeneous_coordinates) است که کاربرد زیادی در گرافیک کامپیوتری دارد. بدون آن، پیاده‌سازی عملیات افاین (affine) مانند عملیات انتقال به عنوان یک ضرب ماتریسی واحد ممکن نبود، زیرا نیاز دارد که یک ثابت به مختصات *اضافه* کنیم. تبدیل افاین در بعد بالاتر به یک تبدیل خطی تبدیل می‌شود!)

بدون متغیر اضافی 1 امکان انتقال وجود ندارد چون نمی توان نقاط را با عددی ثابت جمع کرد. برای فهم بهتر مثال عملیات انتقال زیر را مشاهده کنید.
...
0 &\text{اگر }a = 0 \\
2 \cdot \frac{a}{2} \cdot b &\text{if }a > 0 \text{ , }a \text{ is even} \\
2 \cdot \frac{a-1}{2} \cdot b + b &\text{if }a > 0 \text{ , }a \text{ is odd}
\end{cases}$$
