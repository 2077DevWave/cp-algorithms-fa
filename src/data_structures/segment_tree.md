...
تازه، می‌شه کارهای پیچیده‌تر و کوئری‌های خفن‌تری هم روش پیاده کرد (یه نگاهی به بخش [نسخه‌های پیشرفته درخت بازه ها](segment_tree.md#advanced-versions-of-segment-trees) بنداز).
...
برای همین هم بهش می‌گن «درخت بازه ها»، هرچند که توی بیشتر پیاده‌سازی‌ها، این درخت رو به صورت صریح نمی‌سازیم (یه نگاهی به بخش [پیاده‌سازی](segment_tree.md#implementation) بنداز).

اینجا یه عکس از این درخت بازه ها روی آرایه‌ی $a = [1, 3, -2, 8, -7]$ رو می‌بینی:

!["درخت بازه ها برای مجموع"](sum-segment-tree.png)
...
۱. چه *مقداری* رو توی هر گره درخت بازه ها ذخیره کنیم.
...
!["پرس‌وجوی درخت بازه ها برای مجموع"](sum-segment-tree-query.png)
...
!["به‌روزرسانی درخت بازه ها برای مجموع"](sum-segment-tree-update.png)
...
## <a name="advanced-versions-of-segment-trees"></a>نسخه‌های پیشرفته درخت بازه ها
...
#### شمارش تعداد صفرها و جستجو برای $k$-امین صفر { #counting-zero-search-kth data-toc-label="شمارش تعداد صفرها، جستجو برای k-امین صفر"}
...
ساخت چنین درخت بازه ها‌ای تقریباً به همون روشی که توی مسئله‌ی قبلی انجام شد، انجام می‌شه، فقط الان باید `multiset`ها رو با هم ترکیب کنیم نه لیست‌های مرتب شده.
...
در پایان اشاره می‌کنیم که درخت بازه ها‌ی دوبعدی که به روش توصیف شده فشرده شده، عملاً معادل تغییر درخت بازه ها‌ی یک‌بعدی می‌شه (بخش [ذخیره کل زیرآرایه‌ها در هر رأس](segment_tree.md#saving-the-entire-subarrays-in-each-vertex) رو ببین).
...
با استفاده از این درخت بازه ها می‌تونیم توی زمان $O(\log n)$ موقعیت $k$-امین عنصر رو با استفاده از همون تکنیک مورد بحث در [شمارش تعداد صفرها، جستجو برای $k$-امین صفر](segment_tree.md#counting-zero-search-kth) پیدا کنیم.
...
* [SPOJ - KQUERY](http://www.spoj.com/problems/KQUERY/) [درخت بازه ها‌ی پایدار / درخت مرتب‌سازی ادغامی]
